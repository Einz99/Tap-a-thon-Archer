...
public class BossBehavior : MonoBehaviour
{
    ...
    private void SpawnHomingProjectile(int attackIndex)
    {
        ...
        homingProjectile.homingDuration = 20f; // Time for the projectile to follow the target
        homingProjectile.destroyAfter = 20f; // Time before the projectile self-destructs
    }
    ...
    private IEnumerator SpawnUltimateAttack()
    {
        ...
        float warningRadius = 4f; // Radius around the boss where warning will spawn
        ...
        {
            Vector3 randomPosition = bossPosition.position + new Vector3(Random.insideUnitCircle.x, Random.insideUnitCircle.y, 0f) * warningRadius;
        ...
        List<Vector3> warningPositions = new List<Vector3>(); // Store the warning positions for ultimate spawns
        for (int i = 0; i < 10; i++) // Ensure we match the number of warnings
        {
            Vector3 randomPosition = bossPosition.position + new Vector3(Random.insideUnitCircle.x, Random.insideUnitCircle.y, 0f) * warningRadius;
            warningPositions.Add(randomPosition); // Store the spawn position
        }

        // Step 4: Spawn the ultimate skill prefabs at the same positions as the warnings
